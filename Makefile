...
HTTPS_GIT := https://github.com/Merlin-Network/nemo.git
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -name '*.pb.go' | xargs goimports -w -local github.com/merlin-network/nemo
.PHONY: format
